...
class VehiculoForm(forms.ModelForm):
    ...
    tipo_carroceria = forms.ChoiceField(
        choices=[('SEDAN', 'SEDAN'), ('HATCHBACK', 'HATCHBACK'),
                 ('WAGON', 'WAGON'), ('STATION WAGON', 'STATION WAGON')],
        label="Tipo de Carrocería"
    )

    tipo_servicio = forms.ChoiceField(
        choices=[('PARTICULAR', 'Particular'), ('PUBLICO', 'Público')],
        label="Tipo de Servicio"
    )

    clase = forms.ChoiceField(
        choices=[('AUTOMOVIL', 'Automóvil'), ('CAMIONETA', 'Camioneta')],
        label="Clase"
    )
    ...
    class Meta:
        ...
        fields = [
            'placa', 'marca', 'linea', 'modelo', 'color', 'cilindraje',
            'puertas', 'capacidad', 'clase', 'tipo_carroceria',
            'tipo_servicio', 'valor', 'n_motor',
            'n_chasis', 'n_vin', 'n_serie',
            'sitio_matricula', 'n_acta_matricula',
            'fecha_matricula', 'ciudad_vehiculo',
            'valor_presupuesto', 'imagen'
        ]

        widgets = {
            'fecha_matricula': DateInput(attrs={'type': 'date'}),
            'n_chasis': forms.TextInput(),
            'n_vin': forms.TextInput(),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Bootstrap en TODOS los campos
        for field in self.fields.values():
            field.widget.attrs.update({'class': 'form-control'})

        # Ocultar presupuesto en edición
        if self.instance.pk:
            self.fields['valor_presupuesto'].widget = forms.HiddenInput()
            self.fields['valor_presupuesto'].required = False
...
class FacturaForm(forms.ModelForm):
    class Meta:
        model = Factura
        fields = [
            'n_factura', 'nit', 'nombre_local', 'fecha',
            'descripcion', 'cantidad', 'valor', 'iva', 'imagen'
        ]
        widgets = {
    ...
    def clean(self):
        ...
        if descripcion and self.vehiculo:
            facturas = Factura.objects.filter(id_placa=self.vehiculo)
            for factura in facturas:
                similarity = difflib.SequenceMatcher(
                    None, descripcion, factura.descripcion
                ).ratio()
                if similarity > 0.7:
                    self.add_error(
                        'descripcion',
                        f'Descripción similar a la factura {factura.n_factura}.'
                    )
...
class UsuarioForm(forms.ModelForm):
    class Meta:
        model = usuario
        fields = [
            'nombre', 'p_apellido', 's_apellido', 'cedula',
...
class DocumentosForm(forms.ModelForm):
    placa = forms.CharField(label="Placa del Vehículo")
    ...
    class Meta:
        ...
        widgets = {
            field: DateInput(attrs={'type': 'date'})
            for field in [
                'fecha_expedicion_to', 'fecha_vencimiento_to',
                'fecha_expedicion_soat', 'fecha_vencimiento_soat',
                'fecha_expedicion_tecno', 'fecha_vencimiento_tecno',
                'fecha_expedicion_sRc', 'fecha_vencimiento_sRc'
            ]
